...
#include "llist2.h"
#include "bool.h"
// #include "colors.h" // removed by khkim
...
void proc_client(int *arg)
{
  ...
  send_broadcast_msg("%sUser %s joined the chat.%s\r\n", color_magenta,
		     list_entry->client_info->nickname, color_normal);
  ...
  while (1) {
    ...
    if (ret == -1) {
      ...
      len = recvfrom(sockfd, buffer, sizeof(buffer), 0, 
		     (struct sockaddr *)&list_entry->client_info->address,
		     (socklen_t *)&socklen);
	...
	*pos = 0;
	logline(LOG_DEBUG, "proc_client(): Message buffer contents = %s", ppos);
	logline(LOG_DEBUG, "proc_client(): Complete message received.");
	/* Process message */
	process_msg(ppos, sockfd);
	ppos = pos + 1;
      }
      memset(buffer, 0, 1024);	
      strcpy(buffer, ppos);
